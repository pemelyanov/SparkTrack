...
public class WebAPIModule(IConfiguration configuration) : Module
{
    ...
    private void TryRegisterTelegramBot(ContainerBuilder builder)
    {
        ...
        // События, которые будут обрабатываться TelegramBackgroundEventHandler. Для работы в проекте SparkTrack.Telegram.Core
        // должны быть реализации ITelegramEventHandler для всех перечисленных типов
    ...
    private bool TryRegisterGoogleDrive(ContainerBuilder builder)
    {
        ...
        builder.RegisterType<GoogleDriveFilesService>().As<IGoogleDriveFilesService>().InstancePerDependency();
        builder.RegisterType<GoogleDriveWithLocalDiskBufferingService>().AsImplementedInterfaces().SingleInstance();

        return true;
    }

    private async Task<DriveService> AuthenticateDriveAsync(
