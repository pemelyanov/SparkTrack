    ...
    where TBitmap : class
{
    ...
    protected virtual byte[] GetImageBytes()
    {
        ...
        foreach (var f in _prioritisedFormats)
            if (formats.Contains(f))
                if (TryGetFormatBytes(f.Id, out var bitmap))
                    return bitmap;
        ...
        foreach (var fmt in formats)
        {
            ...
            {
                if (TryGetFormatBytes(orderedFormat.Id, out var dib))
                {
                    if (dib != null)
                    {
                        return dib;
        ...
        if (TryGetFileDropImagePath(out var fileDropImagePath))
            return LoadBytesFromFile(fileDropImagePath);
    ...
    protected byte[] LoadBytesFromFile(string filePath) => File.ReadAllBytes(filePath);
...
/// </summary>
[SupportedOSPlatform("windows")]
public class SimpleClipboardHandle : ClipboardHandle
{
    private List<ClipboardFormat> _prioritisedFormats = new();
    private List<ClipboardFormat> _clipboardOrderFormats = new();
    private List<ClipboardFormat> _otherFormats = new();

    private IEnumerable<ClipboardFormat> _allImageFormats
    {
        get
        {
            foreach (var b in _prioritisedFormats) yield return b;
    ...
    /// Creates a new <see cref="ClipboardHandlePlatformBase{TBitmap}"/>.
    /// </summary>
    public SimpleClipboardHandle()
    {
        var bytesConverter = new BytesDataConverter();

        _prioritisedFormats.Add(ClipboardFormat.Png);
        _prioritisedFormats.Add(ClipboardFormat.CreateCustomFormat("image/png", bytesConverter));

        _clipboardOrderFormats.Add(ClipboardFormat.Dib);
        _clipboardOrderFormats.Add(ClipboardFormat.DibV5);
        _clipboardOrderFormats.Add(ClipboardFormat.Bitmap);

        _otherFormats.Add(ClipboardFormat.Jpeg.WithConverter(bytesConverter));
        _otherFormats.Add(ClipboardFormat.CreateCustomFormat("JPG", bytesConverter));
        _otherFormats.Add(ClipboardFormat.CreateCustomFormat("JFIF", bytesConverter));
        _otherFormats.Add(ClipboardFormat.CreateCustomFormat("image/jpeg", bytesConverter));

        _otherFormats.Add(ClipboardFormat.Tiff.WithConverter(bytesConverter));
        _otherFormats.Add(ClipboardFormat.CreateCustomFormat("image/tiff", bytesConverter));

        _otherFormats.Add(ClipboardFormat.Gif.WithConverter(bytesConverter));
        _otherFormats.Add(ClipboardFormat.CreateCustomFormat("image/gif", bytesConverter));
    }
    ...
    public virtual byte[]? GetImageBytes()
    {
        ...
        foreach (var fmt in formats)
        {
            ...
            if(orderedFormat is null) continue;
